		...
		function actionDrillDown(element, wrapper, obj){
			...
			if(defaults.linkType == 'breadcrumb'){
				...
				if(getNewBreadcrumb == defaults.defaultText){
					$('ul',$header).append('<li><a href="#" class="first">'+defaults.resetText+'</a>if</li>');
				} else {
					$('ul',$header).append('<li><a href="#">'+getNewBreadcrumb+'</a>else</li>');
				}
			}
			if(defaults.linkType == 'backlink'){
				if(!$('a',$header).length){
					$($header).prepend('<a href="#" class="link-back" style="font-size:13px">'+getNewBreadcrumb+'</a>');
				} else {
